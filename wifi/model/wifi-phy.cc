...
{
  WifiPreamble preamble = txVector.GetPreambleType ();
  NS_LOG_INFO(txVector.GetMode ().GetModulationClass ());
  switch (txVector.GetMode ().GetModulationClass ())
    {
    case WIFI_MOD_CLASS_OFDM:
      {
...
{
  WifiPreamble preamble = txVector.GetPreambleType ();
  NS_LOG_INFO(GetPlcpPreambleDuration (txVector));
...
WifiPhy::CalculateTxDuration (uint32_t size, WifiTxVector txVector, uint16_t frequency,
                              MpduType mpdutype, uint8_t incFlag)
{
  NS_LOG_INFO(GetPayloadDuration (size, txVector, frequency, mpdutype, incFlag));
...
{
  NS_LOG_INFO("getmode");
  NS_LOG_INFO("getmode" << m_deviceRateSet.size());
