...
WifiPhyStateHelper::GetDelayUntilIdle (void) const
{
  ...
  NS_LOG_INFO("ceci est le temps d ttente" << retval);
  return retval;
}
...
WifiPhyStateHelper::NotifyMaybeCcaBusyStart (Time duration)
{
  ...
  NS_LOG_INFO(m_listeners.size());

  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
      NS_LOG_FUNCTION ("je passe dans notify");
...
WifiPhyStateHelper::SwitchToRx (Time rxDuration)
{
  ...
  NS_ASSERT (IsStateRx ());
  NS_LOG_INFO(GetState());
   NS_LOG_INFO("test passe");
}

void
WifiPhyStateHelper::SwitchToChannelSwitching (Time switchingDuration)
{
   NS_LOG_FUNCTION ("SwitchToChannelSwitching");
