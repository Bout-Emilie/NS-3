...
#ifndef JAMMING_MITIGATION_H
#define JAMMING_MITIGATION_H
 ...
 */
class JammingMitigation : public Object
{
public:
  static TypeId GetTypeId (void);
  JammingMitigation ();
  virtual ~JammingMitigation ();
   ...
   */
  void StartMitigation (void);

  /**
   * Stops mitigation of jamming.
   */
  void StopMitigation (void);

  void Detection(Ptr<Packet> packet, double startRss);
...
private:
   ...
   */
  virtual void DoMitigation (void) = 0;
   ...
   * Implemented in child classes.
   */
  virtual void DoStopMitigation (void) = 0;
  ...
  virtual void StartDetection(Ptr<Packet> packet, double averageRss) = 0;
...
protected:
  bool IsMitigationOn (void) const;

};

} // namespace ns3

#endif /* JAMMING_MITIGATION_H */
